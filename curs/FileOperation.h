...
const char* N_F = "File not found.";

void writeBitsToFile(const String& filename, const String& bits)
{
...
String readBitsFromFile(const String& filename, size_t size)
{
...
void writeDataToFile(const String& filename, const String& cipher, const Vector<Pair<wchar_t, int>>& alphabet)
{
  ...
  file.imbue(ruLoc);
  file << cipher.data() << std::endl << std::endl;
  for(const auto& pair : alphabet)
  {
    file << pair.first_ << L" " << pair.second_ << std::endl;
  }
  file.close();
}

void readDataFromFile(const String& filename, String& cipher, Vector<Pair<wchar_t, int>>& alphabet)
{
  std::locale ruLoc("ru_RU.UTF-8");
...
void readSourceDoc(const String& filename, String& str)
{
  std::locale ruLoc("ru_RU.UTF-8");
...
void writeDataDecode(const String& filename, const String& decodeStr)
{
  std::locale ruLoc("ru_RU.UTF-8");
  std::wofstream file(filename.data(), std::ios::binary);
  if(!file.is_open())
