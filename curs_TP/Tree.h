...
const std::string E_ER = "The alphabet mast contain at least 2 elements.";
const std::string T_ER = "Tree is empty.";
const std::string S_EM = "Decryption string is empty.";
...
private:
  template<typename T>
   struct Node
   {
     ...
     Node<T> *left_;
     Node<T> *right_;
     Node(const T& data, std::wstring value) : data_(data), value_(value), left_(nullptr), right_(nullptr) {}
   ...
   void buildSubtree(Node<T>*& node, const std::vector<std::pair<wchar_t, int>>& freqs, std::wstring val)
   {
     if(freqs.size() == 1)
     {
       node->data_ = freqs[0].first;
       node->value_ = val;
       return;
     }
     size_t index = 0;
     ...
     int maxSum = 0;
     std::vector<std::pair<wchar_t, int>> vec1;
     std::vector<std::pair<wchar_t, int>> vec2;
     for(size_t i = 0; i < freqs.size(); i++)
     {
       int tempSum = 0;
       for(size_t j = i + 1; j < freqs.size(); j++)
       {
         tempSum += freqs[j].second;
       }
       maxSum += freqs[i].second;
     ...
     for(size_t i = index; i < freqs.size(); i++)
     {
   ...
   bool findElem(Node<T>* node, const wchar_t& value, std::wstring& rez)
   {
     if(node == nullptr)
     {
       return false;
   ...
   void decode(Node<T>* node, const std::wstring& str, std::wstring& rez, size_t& ind)
   {
     if(node == nullptr)
     {
       return;
...
public:
  ...
  void buildTree(const std::vector<std::pair<wchar_t, int>>& frequency)
  {
    if(frequency.size() < 2)
    {
  ...
  std::wstring find(const wchar_t& value)
  {
    std::wstring rez = L"";
